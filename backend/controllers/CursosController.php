...
class CursosController extends Controller
{
                ...
                if (empty($preguntas)) {
                    ...
                    if ($validas) {
                        ...
                        try {
                            ...
                            if (Yii::$app->user->id) {
                                $curso->creado_por = Yii::$app->user->id;
                            }
                            if (!$curso->save()) {
    ...
    // wizard para editar curso existente con sus diapositivas y preguntas en 3 pasos
    public function actionUpdateWizard($id, $step = 1)
    {
        $curso = $this->findModel($id);
        $session = Yii::$app->session;

        // paso 1: editar datos basicos del curso
        if ($step == 1) {
            $model = $curso;
            ...
            if ($this->request->isPost) {
                if ($model->load($this->request->post()) && $model->validate()) {
                    // guardar datos del curso en sesion
                    $session->set('wizard_update_curso_data', $model->attributes);
                    $session->set('wizard_update_curso_id', $id);
                    return $this->redirect(['update-wizard', 'id' => $id, 'step' => 2]);
                }
            } else {
                // cargar desde sesion si existe o desde el modelo
                if ($session->has('wizard_update_curso_data') && $session->get('wizard_update_curso_id') == $id) {
                    $model->attributes = $session->get('wizard_update_curso_data');
                }
            }
        ...
        // paso 2: editar diapositivas
        if ($step == 2) {
            // verificar que paso 1 esta completo
            if (!$session->has('wizard_update_curso_data') || $session->get('wizard_update_curso_id') != $id) {
                Yii::$app->session->setFlash('error', 'debes completar el paso 1 primero');
                return $this->redirect(['update-wizard', 'id' => $id, 'step' => 1]);
            }

            $cursoData = $session->get('wizard_update_curso_data');
            ...
            if ($this->request->isPost) {
                ...
                if (empty($diapositivas)) {
                    ...
                    if ($validas) {
                        // guardar diapositivas en sesion
                        $session->set('wizard_update_diapositivas', $diapositivas);
                        return $this->redirect(['update-wizard', 'id' => $id, 'step' => 3]);
                    } else {
                        Yii::$app->session->setFlash('error', 'todas las diapositivas deben tener titulo y numero de orden');
                    }
                }
            ...
            // cargar diapositivas desde sesion o desde bd
            if ($session->has('wizard_update_diapositivas')) {
                $diapositivas = $session->get('wizard_update_diapositivas');
            } else {
                // cargar diapositivas existentes del curso
                $diapositivasModels = Diapositivas::find()->where(['curso_id' => $id])->orderBy(['numero_orden' => SORT_ASC])->all();
                $diapositivas = [];
                foreach ($diapositivasModels as $diapo) {
                    $diapositivas[] = $diapo->attributes;
                }
            }
        ...
        // paso 3: editar preguntas y guardar todo
        if ($step == 3) {
            // verificar que pasos anteriores estan completos
            if (!$session->has('wizard_update_curso_data') || !$session->has('wizard_update_diapositivas') || $session->get('wizard_update_curso_id') != $id) {
                Yii::$app->session->setFlash('error', 'debes completar los pasos anteriores primero');
                return $this->redirect(['update-wizard', 'id' => $id, 'step' => 1]);
            }

            $cursoData = $session->get('wizard_update_curso_data');
            $diapositivasData = $session->get('wizard_update_diapositivas');
            ...
            if ($this->request->isPost) {
                ...
                if (empty($preguntas)) {
                    ...
                    if ($validas) {
                        ...
                        try {
                            ...
                            if (Yii::$app->user->id) {
                                $curso->modificado_por = Yii::$app->user->id;
                            }
                            if (!$curso->save()) {
                                throw new \Exception('error al actualizar el curso');
                            }

                            // 2. eliminar diapositivas antiguas
                            Diapositivas::deleteAll(['curso_id' => $id]);

                            // 3. crear diapositivas nuevas
                            foreach ($diapositivasData as $diapoData) {
                                $diapositiva = new Diapositivas();
                                $diapositiva->attributes = $diapoData;
                                $diapositiva->curso_id = $curso->id;
                                $diapositiva->fecha_creacion = date('Y-m-d H:i:s');
                                if (Yii::$app->user->id) {
                                    $diapositiva->creado_por = Yii::$app->user->id;
                                }
                                if (!$diapositiva->save()) {
                                    throw new \Exception('error al guardar diapositiva');
                                }
                            ...
                            // 4. eliminar preguntas antiguas
                            PreguntasCuestionario::deleteAll(['curso_id' => $id]);

                            // 5. crear preguntas nuevas
                            foreach ($preguntas as $pregData) {
                                $pregunta = new PreguntasCuestionario();
                                $pregunta->attributes = $pregData;
                                $pregunta->curso_id = $curso->id;
                            ...
                            // limpiar sesion
                            $session->remove('wizard_update_curso_data');
                            $session->remove('wizard_update_diapositivas');
                            $session->remove('wizard_update_curso_id');

                            Yii::$app->session->setFlash('success', 'curso actualizado exitosamente con todas sus diapositivas y preguntas');
                            return $this->redirect(['view', 'id' => $curso->id]);
                ...
                // cargar preguntas existentes del curso si no hay en sesion
                $preguntasModels = PreguntasCuestionario::find()->where(['curso_id' => $id])->all();
                // pre-llenar el formulario con las preguntas existentes via javascript
            }
            ...
            return $this->render('wizard/step3', [
                ...
                'isUpdate' => true,
                'cursoId' => $id,
                'preguntasExistentes' => PreguntasCuestionario::find()->where(['curso_id' => $id])->all(),
            ]);
        }
